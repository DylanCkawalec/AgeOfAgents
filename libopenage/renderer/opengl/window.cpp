...
GlWindow::GlWindow(const std::string &title,
	...
	// Drain any GL errors generated by Qt's internal window setup before
	// validating our own state. On macOS Core profile, Qt's QWindow setup
	// can leave spurious errors (e.g. GL_INVALID_ENUM from queries that are
	// not available in OpenGL 4.1 Core) which are harmless but would
	// otherwise abort `GlContext::check_error()`.
	while (glGetError() != GL_NO_ERROR) {
		// drain pre-init errors so subsequent check_error() calls only see
		// errors caused by our own GL usage.
	}
